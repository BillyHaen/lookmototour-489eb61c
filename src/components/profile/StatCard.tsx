...
export default function StatCard({ icon: Icon, label, value, pulse, accent = 'primary' }: StatCardProps) {
  ...
  return (
    <div className="rounded-xl border border-border bg-card shadow-sm p-4 flex items-center gap-3 min-w-0">
      <div className={cn('relative h-10 w-10 rounded-lg flex items-center justify-center shrink-0', accentClass)}>
      ...
      <div className="min-w-0">
        ...
        <p className="font-heading text-lg font-bold leading-tight truncate">{value}</p>
      </div>
    </div>
  );
}
