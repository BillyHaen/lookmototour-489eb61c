...
export default function Shop() {
  ...
  const { data: products, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await (supabase.from('products') as any).select('*').eq('is_active', true).order('created_at', { ascending: false });
        ...
        <div className="container">
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {filtered.map((p: any) => (
                <div key={p.id} className="rounded-xl bg-card shadow-card border border-border overflow-hidden group hover:shadow-elevated transition-all duration-300 hover:-translate-y-1">
                  <div className="aspect-square overflow-hidden bg-muted">
                    <img src={p.image_url || eventPlaceholder} alt={p.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                  </div>
          ...
          {!isLoading && !filtered.length && <p className="text-muted-foreground text-center py-12">Belum ada produk tersedia.</p>}
        </div>
      </div>
      <Footer />
    </div>
