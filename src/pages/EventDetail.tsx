                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-3 gap-1.5 text-center">
                        {(event as any).price_sharing > 0 && (
                          <div className="p-2 rounded-lg bg-muted min-w-0">
                            <p className="text-[10px] text-muted-foreground uppercase font-medium">Sharing</p>
                            <p className="font-heading font-bold text-xs sm:text-sm text-primary break-words leading-tight">{formatPrice((event as any).price_sharing)}</p>
                          </div>
                        )}
                        {((event as any).price_single > 0 || event.price > 0) && (
                          <div className="p-2 rounded-lg bg-primary/10 border border-primary/20 min-w-0">
                            <p className="text-[10px] text-muted-foreground uppercase font-medium">Single</p>
                            <p className="font-heading font-bold text-xs sm:text-sm text-primary break-words leading-tight">{formatPrice((event as any).price_single || event.price)}</p>
                          </div>
                        )}
                        {(event as any).price_couple > 0 && (
                          <div className="p-2 rounded-lg bg-muted min-w-0">
                            <p className="text-[10px] text-muted-foreground uppercase font-medium">Couple</p>
                            <p className="font-heading font-bold text-xs sm:text-sm text-primary break-words leading-tight">{formatPrice((event as any).price_couple)}</p>
                          </div>
                        )}
                      </div>
