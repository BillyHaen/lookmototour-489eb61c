...
export default function InterestedUsers({ eventId, interestCount }: { eventId: string; interestCount?: number }) {
  const { user } = useAuth();
  ...
  const count = interestedUsers?.length ?? interestCount ?? 0;
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2 text-sm text-muted-foreground">
        ...
        <span>{count} orang berminat</span>
      </div>
      {user && interestedUsers && interestedUsers.length > 0 && (
