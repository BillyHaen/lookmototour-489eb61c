...
export default function RentalCheckoutDialog({ product, trigger }: Props) {
  ...
  const submit = async () => {
    ...
    if (!start || !end || days < 1) {
      toast({ title: 'Pilih tanggal sewa yang valid', variant: 'destructive' });
      return;
    }
    setLoading(true);
    const { error } = await (supabase.from('gear_rentals') as any).insert({
      product_id: product.id,
      user_id: user.id,
      qty,
      daily_price: product.daily_rent_price,
      total_days: days,
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <CalendarDays className="h-5 w-5" /> Sewa: {product.name}
          </DialogTitle>
        </DialogHeader>
        <div className="space-y-3">
          ...
          <div className="p-3 rounded-lg bg-muted/50 space-y-1 text-sm">
            ...
            <div className="flex justify-between font-bold pt-1 border-t border-border"><span>Total Bayar</span><span className="text-primary">{formatPrice(subtotal + deposit)}</span></div>
          </div>
          ...
          <p className="text-[11px] text-muted-foreground text-center">
            Admin akan konfirmasi via WhatsApp. Pembayaran manual.
          </p>
        </div>
      </DialogContent>
    </Dialog>
